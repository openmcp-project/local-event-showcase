import {
  OCMOnboardingService,
  OCMControllerStatus,
} from '../services/ocm-onboarding.service';
    ...
    @switch (state()) {
      @case ('loading') {
        <div class="loading-container">
          ...
          <span>Checking OCM Controller status...</span>
        </div>
      }
      ...
      @case ('configure') {
        <div class="onboarding-card">
          <div class="card-header">
            <fd-icon glyph="cargo-train"></fd-icon>
            <h2>Enable OCM Controller</h2>
          </div>
          <div class="card-description">
            The OCM (Open Component Model) Controller enables software delivery
            and lifecycle management using the Open Component Model in your workspace.
            Select a version to get started.
          </div>
          <div class="config-section">
            <div class="config-field">
              <label>OCM Controller Version</label>
          ...
          <button fd-button label="Install OCM Controller" fdType="emphasized"
      ...
      @case ('creating') {
        <div class="loading-container">
          <fd-busy-indicator [loading]="true" size="m"></fd-busy-indicator>
          <span>Installing OCM Controller...</span>
        </div>
      }
      ...
      @case ('provisioning') {
        <div class="provisioning-card">
          <div class="provisioning-header">
            ...
            <h2>OCM Controller is provisioning</h2>
          </div>
          <div class="provisioning-status">
            <div>
              <div class="phase-text">Your OCM Controller instance is being set up. This may take a few minutes.</div>
              @if (ocmController()?.status?.phase) {
                <div class="phase-label" style="margin-top: 0.5rem">
                  Phase: {{ ocmController()!.status!.phase }}
                </div>
              }
            </div>
          </div>
          @if (ocmController()) {
            <div class="config-section">
              <div class="config-row">
                ...
                <span class="config-value">{{ ocmController()!.spec?.version }}</span>
              </div>
              <div class="config-row">
                <span class="config-label">Status</span>
                <span class="config-value">
                  <span class="status-badge provisioning">{{ ocmController()!.status?.phase ?? 'Pending' }}</span>
                </span>
              </div>
            </div>
          }
          ...
          @if (ocmController()) {
            <div class="config-section">
              ...
              <div class="config-row">
                <span class="config-label">Status</span>
                <span class="config-value">
                  <span class="status-badge">{{ ocmController()!.status?.phase ?? 'Unknown' }}</span>
                </span>
              </div>
            </div>
          }
          <fd-message-strip type="success" [dismissible]="false">
            OCM Controller is installed and running in your workspace.
          </fd-message-strip>
        </div>
      }
    }
...
export class OCMOnboardingComponent implements OnDestroy {
  private onboardingService = inject(OCMOnboardingService);
  private watchSub?: Subscription;
  private luigiContext!: LuigiContext;

  readonly availableVersions = [{ version: 'v0.29.0', chartVersion: '0.0.0-6205a8a' }];

  state = signal<OnboardingState>('loading');
  error = signal('');
  ocmController = signal<OCMControllerStatus | null>(null);
  selectedVersion = signal('v0.29.0');
  selectedChartVersion = signal('0.0.0-6205a8a');
  ...
  onConfigure(): void {
    ...
    this.error.set('');
    this.onboardingService.createOCMController(this.selectedVersion(), this.selectedChartVersion() || undefined).subscribe({
      next: () => this.startWatching(),
      error: (err) => {
        this.error.set(`Failed to create OCM Controller: ${err.message}`);
  ...
  private checkState(): void {
    ...
    this.onboardingService.checkOCMController().subscribe({
      next: (resource) => {
        if (resource) {
          this.ocmController.set(resource);
  ...
  private startWatching(): void {
    ...
    this.watchSub?.unsubscribe();
    this.watchSub = this.onboardingService.watchOCMController().subscribe({
      next: (event) => {
