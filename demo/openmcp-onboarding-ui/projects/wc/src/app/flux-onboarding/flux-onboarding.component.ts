import {
  FluxOnboardingService,
  FluxStatus,
} from '../services/flux-onboarding.service';
    ...
    @switch (state()) {
      @case ('loading') {
        <div class="loading-container">
          ...
          <span>Checking Flux status...</span>
        </div>
      }
      ...
      @case ('configure') {
        <div class="onboarding-card">
          <div class="card-header">
            <fd-icon glyph="source-code"></fd-icon>
            <h2>Enable Flux</h2>
          </div>
          <div class="card-description">
            Flux is a GitOps toolkit for continuous delivery on Kubernetes.
            It keeps your clusters in sync with sources of configuration and automates
            updates when new code is available.
          </div>
          <div class="config-section">
            <div class="config-field">
              <label>Flux Version</label>
          ...
          <button fd-button label="Install Flux" fdType="emphasized"
      ...
      @case ('provisioning') {
        <div class="provisioning-card">
          <div class="provisioning-header">
            ...
            <h2>Flux is provisioning</h2>
          </div>
          <div class="provisioning-status">
            <div>
              <div class="phase-text">Your Flux instance is being set up. This may take a few minutes.</div>
              @if (flux()?.status?.phase) {
                <div class="phase-label" style="margin-top: 0.5rem">
                  Phase: {{ flux()!.status!.phase }}
                </div>
              }
            </div>
          </div>
          @if (flux()) {
            <div class="config-section">
              <div class="config-row">
                ...
                <span class="config-value">{{ flux()!.spec?.version }}</span>
              </div>
              <div class="config-row">
                <span class="config-label">Status</span>
                <span class="config-value">
                  <span class="status-badge provisioning">{{ flux()!.status?.phase ?? 'Pending' }}</span>
                </span>
              </div>
            </div>
          }
          ...
          @if (flux()) {
            <div class="config-section">
              ...
              <div class="config-row">
                <span class="config-label">Status</span>
                <span class="config-value">
                  <span class="status-badge">{{ flux()!.status?.phase ?? 'Unknown' }}</span>
                </span>
              </div>
            </div>
          }
          <fd-message-strip type="success" [dismissible]="false">
            Flux is installed and running in your workspace.
          </fd-message-strip>
        </div>
      }
    }
...
export class FluxOnboardingComponent implements OnDestroy {
  private onboardingService = inject(FluxOnboardingService);
  private watchSub?: Subscription;
  private luigiContext!: LuigiContext;

  readonly availableVersions = [{ version: 'v2.4.0', chartVersion: '2.14.0' }];

  state = signal<OnboardingState>('loading');
  error = signal('');
  flux = signal<FluxStatus | null>(null);
  selectedVersion = signal('v2.4.0');
  selectedChartVersion = signal('2.14.0');
  ...
  onConfigure(): void {
    ...
    this.onboardingService
      .createFlux(this.selectedVersion(), this.selectedChartVersion() || undefined)
      .subscribe({
        next: () => this.startWatching(),
        error: (err) => {
          this.error.set(`Failed to create Flux: ${err.message}`);
  ...
  private checkState(): void {
    ...
    this.onboardingService.checkFlux().subscribe({
      next: (resource) => {
        if (resource) {
          this.flux.set(resource);
  ...
  private startWatching(): void {
    ...
    this.watchSub?.unsubscribe();
    this.watchSub = this.onboardingService.watchFlux().subscribe({
      next: (event) => {
        this.flux.set(event.object);
  ...
  private sendPortalReloadMessage(): void {
    ...
    sendCustomMessage({
      ...
      origin: 'FluxOnboarding',
      action: 'provisionFlux',
      entity: entityType,
      context: {
